...
internal sealed class DiagnosticsViewModel : ReactiveObject, IActivatableViewModel, IDisposable
{
    ...
    public DiagnosticsViewModel(DiagnosticLog log)
    {
        ...
        this.WhenActivated(disposables =>
        {
            ...
            disposables(Scope.Create(() => _log.Info("Reactive activation stopped; subscriptions disposed.")));
        });
    }
    ...
    private void ObserveCommandState(Action<IDisposable> register)
    {
        ...
        register(Wait.ThrownExceptions
            .Keep(static exception => exception is not OperationCanceledException)
    ...
    private void ObserveWindowsEvents(Action<IDisposable> register)
    {
        register(ObserveSafely(
            ...
            "display topology"));
        register(Signal.Every(TimeSpan.FromSeconds(1))
            .Map(static _ => ClipboardNative.SequenceNumber)
            .Unique()
            .Skip(1)
            .Subscribe(
    ...
    private void ObserveCursor(Action<IDisposable> register) =>
        register(Signal.Every(TimeSpan.FromMilliseconds(500))
            .Map(static _ => User32Api.GetCursorLocation())
            .Unique()
            .Subscribe(
    ...
    private void ObserveAutomaticRefresh(Action<IDisposable> register) =>
        register(Signal.Every(TimeSpan.FromSeconds(3))
            .Keep(_ => IsWatching && !IsBusy)
            .FlatMap(_ => Refresh.Execute().Recover<DiagnosticSnapshot, Exception>(exception =>
            {
                _log.Warning($"Automatic refresh skipped: {exception.Message}");
                return Signal.None<DiagnosticSnapshot>();
            }))
            .Subscribe());
