...
internal sealed class ConsoleDashboard : IDisposable
{
    ...
    private async Task DispatchAsync(string input, CancellationToken cancellationToken)
    {
        ...
        switch (command)
        {
            ...
            case "windows":
                ...
                break;
            case "reactive":
                await RenderReactiveShimAsync(ParseCount(parts), cancellationToken).ConfigureAwait(false);
                break;
            case "displays":
                RenderDisplays();
                break;
    ...
    private async Task RenderReactiveShimAsync(int count, CancellationToken cancellationToken)
    {
        var rows = await DiagnosticsService.GetReactiveShimHandlesAsync(count, cancellationToken).ConfigureAwait(false);
        WriteLine("Handles streamed by CP.ReactiveUI.Primitives.Windows.Reactive:", ConsoleColor.Cyan);
    ...
    private static void RenderAbout()
    {
        ...
        WriteLine("  System.Reactive pipelines over local Windows.Reactive, Core, and Integrations APIs");
    }
    ...
    private static void RenderHelp(bool compact)
    {
        ...
        WriteLine("  reactive [n]                  Stream handles through the local Reactive shim");
    ...
    private static void RenderBanner()
    {
        ...
        WriteLine("║  CP.REACTIVEUI.PRIMITIVES.WINDOWS.REACTIVE  •  RX DIAGNOSTICS     ║", ConsoleColor.Cyan);
        WriteLine("╚══════════════════════════════════════════════════════════════════════╝", ConsoleColor.Cyan);
    }
