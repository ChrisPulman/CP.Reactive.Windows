...
using ReactiveWindowsEnumerator = CP.ReactiveUI.Primitives.Windows.Reactive.Desktop.Windows.WindowsEnumerator;
...
internal static class DiagnosticsService
{
    ...
    /// <summary>Enumerates window handles through the System.Reactive-flavoured local project.</summary>
    ...
    public static async Task<IReadOnlyList<string>> GetReactiveShimHandlesAsync(int count, CancellationToken cancellationToken)
    {
        IList<string> handles = await ReactiveWindowsEnumerator.ObserveWindowHandles()
            .Take(Math.Max(1, count))
            .Select(static handle => $"0x{handle.ToInt64():X}")
            .ToList()
