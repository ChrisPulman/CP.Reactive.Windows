...
public sealed class MainViewModel : ReactiveObject, IActivatableViewModel
{
    ...
    internal MainViewModel(IWindowsLabService service)
    {
        ...
        _mainScheduler = new SynchronizationContextScheduler(
        ...
        this.WhenActivated((Action<IDisposable> disposables) =>
        {
            ...
            this.WhenAnyValue(static viewModel => viewModel.EventFilter)
                .Throttle(TimeSpan.FromMilliseconds(160), TaskPoolScheduler.Default)
                .ObserveOn(_mainScheduler)
    ...
    public Interaction<Unit, ImageSource> CaptureDashboard { get; } = new();

    /// <summary>Gets the command that refreshes local HWND, DPI, display and foreground metrics.</summary>
    public ReactiveCommand<Unit, Unit> RefreshEnvironmentCommand { get; }

    /// <summary>Gets the command that refreshes only the foreground window.</summary>
    public ReactiveCommand<Unit, Unit> RefreshForegroundCommand { get; }
    ...
    public ReactiveCommand<Unit, Unit> CaptureCursorCommand { get; }

    /// <summary>Gets the command that renders this dashboard to memory.</summary>
    public ReactiveCommand<Unit, Unit> CaptureDashboardCommand { get; }
    ...
    internal IObservable<Unit> CreateWindowGuard(WindowGuardMode mode) => _service.CreateWindowGuard(mode);
    ...
    private static IObservable<LabEvent> SafeStream(IObservable<LabEvent> source, string sourceName) =>
        source.Catch<LabEvent, Exception>(exception =>
            Observable.Return(new LabEvent(sourceName, $"Unavailable: {exception.Message}")));
    ...
    private void RefreshEnvironment()
    {
        try
        {
            ...
            StatusText = "Environment refreshed through Core and Reactive Windows APIs.";
    ...
    private async Task CaptureDashboardAsync()
    {
        try
        {
            DashboardPreview = await CaptureDashboard.Handle(Unit.Default);
