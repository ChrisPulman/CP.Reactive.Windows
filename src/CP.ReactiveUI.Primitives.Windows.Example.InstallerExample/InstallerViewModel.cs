...
{
    private readonly Signal<DeploymentEvent> _events = new();
    ...
    public InstallerViewModel()
    {
        ...
        RunReadOnlyProbe = ReactiveCommand.CreateFromTask(RunReadOnlyProbeAsync, RunWorkflow.IsExecuting.Map(static running => !running));
        RevealPackage = ReactiveCommand.Create(RevealPackageInExplorer, this.WhenAnyValue(x => x.PackagePath).Map(File.Exists));
    ...
    private static EnvironmentSnapshot CreateEnvironmentSnapshot()
    {
        ...
        var visibleWindowSample = CountLeanWindowStream();
    ...
    private static int CountLeanWindowStream()
    {
