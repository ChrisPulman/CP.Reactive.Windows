...
internal sealed class OperationsCenterViewModel : ReactiveObject, IDisposable
{
    ...
    private readonly MultipleDisposable _lifetime = new MultipleDisposable();
    ...
    public OperationsCenterViewModel(WindowsOperationsService service)
    {
        ...
        var hasWindow = this.WhenAnyValue(viewModel => viewModel.SelectedWindow).Map(window => window != null);
    ...
    public IDisposable Activate(Control control)
    {
        if (control is null)
        {
        ...
        var activation = new MultipleDisposable();
        var inputSubscription = new MutableDisposable();
        activation.Add(inputSubscription);
        var keepAwakeWasEnabled = false;
        var uiSequencer = new ControlSequencer(control);
        ...
        activation.Add(this.WhenAnyValue(viewModel => viewModel.InputCaptureEnabled)
            ...
            .Subscribe(enabled =>
            {
                inputSubscription.Disposable = enabled ? _service.StartInputCapture() : EmptyDisposable.Instance;
            }));
        ...
        activation.Add(Signal.Every(TimeSpan.FromSeconds(1D), uiSequencer)
            .StartWith(0L)
            .Subscribe(_ => UpdateInputIdle()));

        activation.Add(new ActionDisposable(() =>
        {
    ...
    private void UpdateInputIdle()
    {
        ...
        {
            var idle = CP.ReactiveUI.Primitives.Windows.Desktop.Input.NativeInput.LastInputTimeSpan;
