...
internal sealed class ClipboardSemaphore : IDisposable
{
    ...
    private sealed class ClipboardSemaphoreOperations(Func<IntPtr, bool> openClipboard, Func<bool> closeClipboard, Func<int, TimeSpan, IntPtr?> tryAcquireMessageWindow)
    {
        /// <summary>Closes the clipboard.</summary>
    ...
    private static readonly Lazy<IDisposable> MessageWindowLifetime = new(() => SharedMessageWindow.ObserveWindowMessages().Subscribe(GC.KeepAlive));

    /// <summary>Native clipboard semaphore operations used by this type.</summary>
    private static ClipboardSemaphoreOperations _operations = new(NativeMethods.OpenClipboard, NativeMethods.CloseClipboard, TryAcquireSharedMessageWindow);

    /// <summary>Shared message-window operations used by the default acquisition path.</summary>
    private static SharedMessageWindowOperations _sharedMessageWindowOperations = new(() => MessageWindowLifetime.Value, () => SharedMessageWindow.NativeHandle);
    ...
    internal static IDisposable OverrideOperationsForTesting(Func<IntPtr, bool> openClipboard, Func<bool> closeClipboard, Func<int, TimeSpan, IntPtr?> tryAcquireMessageWindow)
    {
        ...
        return Scope.Create(operations, delegate(ClipboardSemaphoreOperations previous)
        {
    ...
    internal static IDisposable OverrideSharedMessageWindowOperationsForTesting(Func<IDisposable> keepAlive, Func<IntPtr> getNativeHandle)
    {
        ...
        return Scope.Create(sharedMessageWindowOperations, delegate(SharedMessageWindowOperations previous)
        {
            _sharedMessageWindowOperations = previous;
        });
    }
    ...
    internal IClipboardAccessToken Lock() => Lock((IntPtr)0, 5, DefaultRetryInterval, DefaultTimeout);

    /// <summary>Get a lock to the clipboard.</summary>
    /// <param name="windowHandle">IntPtr with a window handle for the potential new owner.</param>
    /// <returns>IClipboardLock.</returns>
    internal IClipboardAccessToken Lock(IntPtr windowHandle) => Lock(windowHandle, 5, DefaultRetryInterval, DefaultTimeout);
    ...
    internal ValueTask<IClipboardAccessToken> LockAsync() => LockAsync((IntPtr)0, 5, DefaultRetryInterval, DefaultTimeout, CancellationToken.None);

    /// <summary>Lock the clipboard, return a disposable which can free this again.</summary>
    /// <param name="cancellationToken">CancellationToken.</param>
    /// <returns>Task with IClipboardLock.</returns>
    internal ValueTask<IClipboardAccessToken> LockAsync(CancellationToken cancellationToken) => LockAsync((IntPtr)0, 5, DefaultRetryInterval, DefaultTimeout, cancellationToken);

    /// <summary>Lock the clipboard, return a disposable which can free this again.</summary>
    /// <param name="windowHandle">IntPtr with the window handle of the potential new owner.</param>
    /// <returns>Task with IClipboardLock.</returns>
    internal ValueTask<IClipboardAccessToken> LockAsync(IntPtr windowHandle) => LockAsync(windowHandle, 5, DefaultRetryInterval, DefaultTimeout, CancellationToken.None);
    ...
    /// <param name="retryInterval">TimeSpan for the time between retries.</param>
    /// <returns>Task with IClipboardLock.</returns>
    internal ValueTask<IClipboardAccessToken> LockAsync(IntPtr windowHandle, int retries, TimeSpan retryInterval) => LockAsync(windowHandle, retries, retryInterval, DefaultTimeout, CancellationToken.None);
    ...
    /// <param name="timeout">TimeSpan for the timeout.</param>
    /// <returns>Task with IClipboardLock.</returns>
    internal ValueTask<IClipboardAccessToken> LockAsync(IntPtr windowHandle, int retries, TimeSpan retryInterval, TimeSpan timeout) => LockAsync(windowHandle, retries, retryInterval, timeout, CancellationToken.None);
    ...
    /// <param name="cancellationToken">CancellationToken.</param>
    /// <returns>Task with IClipboardLock.</returns>
    internal ValueTask<IClipboardAccessToken> LockAsync(IntPtr windowHandle, int retries, TimeSpan retryInterval, TimeSpan timeout, CancellationToken cancellationToken)
    {
        if (!cancellationToken.IsCancellationRequested)
        {
            return new(Lock(windowHandle, retries, retryInterval, timeout));
        }

        return new(Task.FromCanceled<IClipboardAccessToken>(cancellationToken));
    }
    ...
    /// </summary>
    /// <param name="newOwnerWindowHandle">IntPtr with the window handle of the new owner. If this parameter is NULL, the open clipboard is associated with the current task.</param>
    ...
    private static bool TryAcquireMessageWindow(int retries, TimeSpan retryInterval, out IntPtr windowHandle) => _operations.TryAcquireMessageWindow(retries, retryInterval, out windowHandle);
    ...
    private ClipboardAccessToken CreateAccessToken() => new(delegate
        {
            _ = CloseClipboard();
            _ = _semaphoreSlim.Release();
        });
