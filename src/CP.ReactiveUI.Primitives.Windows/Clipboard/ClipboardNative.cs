...
public static class ClipboardNative
{
    ...
    private sealed class ClipboardNativeOperations(Func<IntPtr> getOwner, Func<uint> getSequenceNumber, Func<uint, bool> isFormatAvailable)
    {
    ...
    private static ClipboardNativeOperations _operations = new(NativeMethods.GetClipboardOwner, NativeMethods.GetClipboardSequenceNumber, NativeMethods.IsClipboardFormatAvailable);
    ...
    /// </summary>
    public static IObservable<ClipboardUpdateInformation> ClipboardUpdateEvents => _clipboardUpdateEvents ??= ReactiveSignal.Create((IObserver<ClipboardUpdateInformation> observer) => (from m in SharedMessageWindow.ObserveWindowMessages(
        delegate (long hwnd)
        {
        ...
        delegate (long hwnd)
        {
            _ = NativeMethods.RemoveClipboardFormatListener((nint)hwnd);
        })
                                                                                                                                                                                                                  where m.Msg == WindowsMessages.WM_CLIPBOARDUPDATE
        select m).Subscribe(
        delegate (WindowMessage m)
    {
        ClipboardUpdateInformation clipboardUpdateInformation = ClipboardUpdateInformation.Create((nint)m.Hwnd);
        if (clipboardUpdateInformation.Id > _previousSequence)
        {
            _previousSequence = clipboardUpdateInformation.Id;
    ...
    /// Best to use SubscribeOn with the UI SynchronizationContext.
    /// </summary>
    public static IObservable<ClipboardRenderFormatRequest> ClipboardRenderFormatRequests => _clipboardRenderFormatRequests ??= ReactiveSignal.Create((IObserver<ClipboardRenderFormatRequest> observer) => (from m in SharedMessageWindow.ObserveWindowMessages()
        where m.Msg == WindowsMessages.WM_RENDERALLFORMATS || m.Msg == WindowsMessages.WM_RENDERFORMAT || m.Msg == WindowsMessages.WM_DESTROYCLIPBOARD
        select m).Subscribe(
        delegate (WindowMessage m)
    {
        ClipboardRenderFormatRequest value = m.Msg switch
        {
            ...
            {
                RequestedFormatId = checked((uint)m.WParam),
            ...
            _ => throw new InvalidOperationException($"Unsupported clipboard message {m.Msg}."),
        };
    ...
    /// <returns>IClipboard, which will unlock when Dispose is called.</returns>
    public static IClipboardAccessToken Access(IntPtr windowHandle, int retries, TimeSpan retryInterval, TimeSpan timeout) => ClipboardLockProvider.Lock(windowHandle, retries, retryInterval, timeout);
    ...
    /// <returns>IClipboard in a Task, which will unlock when Dispose is called.</returns>
    public static ValueTask<IClipboardAccessToken> AccessAsync(IntPtr windowHandle, int retries, TimeSpan retryInterval, TimeSpan timeout) => ClipboardLockProvider.LockAsync(windowHandle, retries, retryInterval, timeout);
    ...
    /// <param name="cancellationToken">CancellationToken.</param>
    /// <returns>IClipboard in a Task, which will unlock when Dispose is called.</returns>
    public static ValueTask<IClipboardAccessToken> AccessAsync(IntPtr windowHandle, int retries, TimeSpan retryInterval, TimeSpan timeout, CancellationToken cancellationToken) => ClipboardLockProvider.LockAsync(windowHandle, retries, retryInterval, timeout, cancellationToken);
    ...
    internal static IDisposable OverrideOperationsForTesting(Func<IntPtr> getOwner, Func<uint> getSequenceNumber, Func<uint, bool> isFormatAvailable)
    {
        ...
        return Scope.Create(operations, delegate(ClipboardNativeOperations previous)
        {
