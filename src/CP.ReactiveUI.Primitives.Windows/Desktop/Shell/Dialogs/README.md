# CP.ReactiveUI.Primitives.Windows.Dialogs
...
`CP.ReactiveUI.Primitives.Windows.Dialogs` wraps the Windows **Common Item Dialog** (Vista+, `IFileOpenDialog` / `IFileSaveDialog`) in two layers:
...
```csharp
using CP.ReactiveUI.Primitives.Windows.Dialogs;
