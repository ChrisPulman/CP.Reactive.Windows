...
public static class SharedMessageWindow
{
    ...
    private readonly struct WindowClassEx
    {
    ...
    private static class NativeMethods
    {
        ...
        private static readonly nint RegisterClassExExport = System.Runtime.InteropServices.NativeLibrary.GetExport(User32Module, "RegisterClassExW");

        /// <summary>The DefWindowProcW export pointer.</summary>
        private static readonly nint DefWindowProcExport = System.Runtime.InteropServices.NativeLibrary.GetExport(User32Module, "DefWindowProcW");

        /// <summary>The PostQuitMessage export pointer.</summary>
        private static readonly nint PostQuitMessageExport = System.Runtime.InteropServices.NativeLibrary.GetExport(User32Module, "PostQuitMessage");
        ...
        public static unsafe ushort RegisterClassEx(WndProc windowProcedure, nint instanceHandle, string className)
        {
            fixed (char* classNamePointer = className)
            {
                ...
                return ((delegate* unmanaged[Stdcall]<WindowClassEx*, ushort>)checked((nuint)RegisterClassExExport))(&windowClass);
            }
        }
        ...
        [DefaultDllImportSearchPaths(DllImportSearchPath.System32)]
        internal static extern nint CreateWindowEx(uint extendedStyle, string className, string windowName, uint style, int x, int y, int width, int height, nint parentWindowHandle, nint menuHandle, nint instanceHandle, nint parameter);
        ...
        internal static unsafe nuint DefWindowProc(nint windowHandle, WindowsMessages message, nint wordParam, nint longParam) => ((delegate* unmanaged[Stdcall]<IntPtr, WindowsMessages, IntPtr, IntPtr, UIntPtr>)checked((nuint)DefWindowProcExport))(windowHandle, message, wordParam, longParam);

        /// <summary>Posts a quit message.</summary>
        /// <param name="exitCode">The exit code.</param>
        internal static unsafe void PostQuitMessage(int exitCode) => ((delegate* unmanaged[Stdcall]<int, void>)checked((nuint)PostQuitMessageExport))(exitCode);
        ...
        [DefaultDllImportSearchPaths(DllImportSearchPath.System32)]
        [return: MarshalAs(UnmanagedType.Bool)]
        internal static extern bool PostMessage(nint windowHandle, uint message, nint wordParam, nint longParam);
    }
    ...
    private sealed class MessageSubscriptionState(IObserver<WindowMessage> observer, string className)
    {
        ...
        public void DisposeCore()
        {
            ...
            if (windowHandle != 0)
            {
                _ = NativeMethods.PostMessage(windowHandle, 2U, 0, 0);
            }
        }
    ...
    /// <param name="onTeardown">Invoked with the window handle value when the window is destroyed or when the subscription is disposed.</param>
    /// <returns>An observable sequence of Windows messages.</returns>
    public static IObservable<WindowMessage> ObserveWindowMessages(Action<long> onSetup, Action<long> onTeardown) => ListenCore(
        delegate (nint windowHandle)
        {
    ...
    internal static IDisposable OverrideStreamsForTesting(IObservable<WindowMessage> messages, IObservable<nint> handleChanges, nint currentHandle)
    {
        ...
        return Scope.Create((previousMessages, previousHandleChanges, previousHandle), delegate((IObservable<WindowMessage> previousMessages, IObservable<nint> previousHandleChanges, nint? previousHandle) previous)
        {
            lock (SyncRoot)
            {
    ...
    /// <param name="longParam">The long parameter.</param>
    /// <returns>The message result.</returns>
    internal static nuint ProcessWindowMessageForTesting(IObserver<WindowMessage> observer, nint windowHandle, WindowsMessages message, nint wordParam, nint longParam) => ProcessWindowMessage(observer, windowHandle, message, wordParam, longParam);
    ...
    /// <param name="onTeardown">The teardown callback.</param>
    /// <returns>An observable sequence of Windows messages.</returns>
    private static IObservable<WindowMessage> ListenCore(Action<nint> onSetup, Action<nint> onTeardown) => ReactiveSignal.Create(delegate(IObserver<WindowMessage> observer)
        {
            ListenState state = new(onSetup, onTeardown);
            IDisposable handleSubscription = ObserveNativeHandleChanges().Subscribe(delegate(nint windowHandle)
            {
            ...
            return Scope.Create(state, delegate(ListenState listenState)
            {
    ...
    private static IDisposable CreateMessageSubscription(IObserver<WindowMessage> observer)
    {
        ...
        Thread thread = new(delegate(object threadState)
        {
        ...
        thread.Start(state);
        return Scope.Create(state, delegate(MessageSubscriptionState subscriptionState)
        {
    ...
    private static void RunMessageLoop(MessageSubscriptionState state)
    {
        ...
        nint windowHandle = NativeMethods.CreateWindowEx(262_272U, className, "CP.ReactiveUI.Primitives.Windows.MessageWindow", 2_147_483_648U, 0, 0, 0, 0, 0, 0, instanceHandle, 0);
        if (windowHandle != 0)
        {
    ...
    private static nuint ProcessWindowMessage(IObserver<WindowMessage> observer, nint windowHandle, WindowsMessages message, nint wordParam, nint longParam)
    {
        ...
        {
            return NativeMethods.DefWindowProc(windowHandle, message, wordParam, longParam);
        }
